...
head(d)
summary(d)
d$Gp = ifelse(d$Gp == "RstZsatTable", "No Calculate Value", "Calculate Value")
library(ggplot2)
library(plyr)
library(dplyr)
...
ggplot(data = d %>% 
  ...
  geom_text(data = lab_df, aes(label = lab), x = 1000000, y = 75,  parse = TRUE)+
  facet_grid(Gp ~. )+
  scale_shape_manual(values = c(21,22), name = "The polygon is simple")+
